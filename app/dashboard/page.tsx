...
export default function DashboardPage() {
  ...
  const acciones = isCorporativo ? [
    { title: 'Agregar proveedor', desc: 'Invitá proveedores a tu red', icon: '➕' },
    { title: 'Aprobar facturas', desc: 'Revisá solicitudes pendientes', icon: '✅' },
    { title: 'Ver reportes', desc: 'Análisis de financiamiento', icon: '📈' },
  ] : [
    { title: 'Cargar factura', desc: 'Subí una nueva factura para financiar', icon: '📤' },
    { title: 'Ver mis facturas', desc: 'Estado de tus solicitudes', icon: '🗂️' },
  ]
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className={`grid ${isCorporativo ? 'md:grid-cols-3' : 'md:grid-cols-2'} gap-4 mb-10`}>
          {acciones.map(item => (
            <div key={item.title} className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-slate-600 transition-colors cursor-pointer">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-800">
            <h2 className="text-white font-semibold">
              {isCorporativo ? 'Proveedores recientes' : 'Mis facturas'}
            </h2>
          </div>
          <div className="py-16 text-center">
            <p className="text-4xl mb-3">{isCorporativo ? '🏭' : '📄'}</p>
            <p className="text-slate-400">{isCorporativo ? 'No hay proveedores aún' : 'No hay facturas cargadas'}</p>
            <p className="text-slate-500 text-sm mt-1">{isCorporativo ? 'Invitá proveedores para comenzar' : 'Cargá tu primera factura para comenzar'}</p>
          </div>
        </div>
      </main>
    </div>
