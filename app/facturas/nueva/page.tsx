...
export default function NuevaFactura() {
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    try {
      ...
      setDebug(prev => prev + '\nGemini OK: ' + JSON.stringify(datosExtraidos))

      setDebug(prev => prev + '\nInsertando en DB...')
      const { data, error: dbError } = await supabase.from('facturas').insert({
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          <Link href="/dashboard" className="text-slate
Last login: Sun Apr 12 13:05:42 on ttys000
federamirez@MacBookAir ~ % >....                                                
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="w-full max-w-lg">
          <div className="bg-slate-900 border border-emerald-500/30 rounded-2xl p-8 text-center">
            <div className="text-5xl mb-3">✅</div>
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          <Link href="/dashboard" className="text-slate
cmdand heredoc> 
cd "/Users/federamirez/Desktop/sk project/scf-latam" && cat > app/facturas/nueva/page.tsx << 'EOF'
'use client'
import { useState } from 'react'
import { supabase } from '@/lib/supabase'
import Link from 'next/link'
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        ...
        <form onSubmit={handleUpload} className="bg-slate-900 border border-slate-800 rounded-2xl p-8 space-y-6">
          ...
          {debug && (
            <div className="bg-slate-800 rounded-lg px-4 py-3 text-xs text-slate-300 whitespace-pre-wrap">
              {debug}
            </div>
          )}
          <div
